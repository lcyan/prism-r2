                    ...
                    {viewMode === 'list' ? (
                        <div className="space-y-4">
                            {sortedFiles.map(file => {
                                ...
                                return (
                                    <div key={file.key} className="group relative bg-white/50 dark:bg-white/[0.02] border border-white/20 dark:border-white/5 rounded-[2rem] p-5 hover:bg-white/80 dark:hover:bg-white/[0.05] hover:shadow-xl hover:-translate-y-1 transition-all duration-500">
                                        <div className="flex items-start justify-between">
                                            <div className="flex items-start gap-4 flex-1">
                                                <div className="mt-1 flex-shrink-0">
                                                    <button onClick={() => toggleSelect(file.key)} className={`w-6 h-6 rounded-xl flex items-center justify-center transition-all duration-300 ${selectedKeys.includes(file.key) ? 'bg-primary text-white border-none shadow-md' : 'bg-white/50 dark:bg-white/5 border-2 border-gray-100 dark:border-white/10 text-transparent group-hover:border-primary/50'}`}><Check size={14} /></button>
                                                </div>
                                                <div className="flex items-center gap-5 flex-1">
                                                    <div className="w-20 h-20 rounded-2xl bg-white/50 dark:bg-white/5 flex items-center justify-center overflow-hidden cursor-pointer hover:ring-4 hover:ring-primary/20 transition-all shadow-inner border border-white/20" onClick={() => setPreviewFile(file)}>{isImage(file.name) ? (<img src={publicUrlGetter(file.key)} alt={file.name} className="w-full h-full object-cover" loading="lazy" />) : (<FileIcon size={32} className="text-amber-500" />)}</div>
                                            ...
                                            <div className="flex items-center gap-2">
                                                <button onClick={() => setPreviewFile(file)} className="w-10 h-10 flex items-center justify-center rounded-xl text-gray-400 hover:text-purple-500 hover:bg-purple-50 dark:hover:bg-purple-500/10 transition-all"><Eye size={18} /></button>
                                                ...
                                                <button onClick={() => handleCopy(publicUrlGetter(file.key), 'url')} className="w-10 h-10 flex items-center justify-center rounded-xl text-gray-400 hover:text-primary hover:bg-primary/5 dark:hover:bg-primary/10 transition-all"><Copy size={18} /></button>
                            ...
                            {sortedFiles.map(file => {
                                ...
                                return (
                                    <div key={file.key} className="group flex flex-col animate-slide-up">
                                        {/* Card Body */}
                                        <div className="w-full bg-white/50 dark:bg-white/[0.02] backdrop-blur-sm rounded-[2rem] p-5 shadow-[0_4px_20px_0_rgba(0,0,0,0.03)] border border-white/20 dark:border-white/5 flex flex-col gap-5 group-hover:shadow-2xl group-hover:-translate-y-2 transition-all duration-500 relative">
                                            {/* Thumbnail Container */}
                                            <div className="w-full aspect-video rounded-2xl bg-white/50 dark:bg-zinc-800/50 flex items-center justify-center relative overflow-hidden ring-1 ring-black/5 shadow-inner">
                                                {isImage(file.name) ? (
                                                    <img src={currentUrl} alt={file.name} className="w-full h-full object-cover cursor-pointer hover:scale-110 transition-transform duration-700" onClick={() => setPreviewFile(file)} />
                                                ) : (
                                                ...
                                                <button
                                                    ...
                                                    className={`absolute top-4 left-4 w-8 h-8 rounded-xl flex items-center justify-center transition-all z-10 ${selectedKeys.includes(file.key) ? 'bg-primary text-white shadow-lg scale-110' : 'bg-black/20 backdrop-blur-md text-transparent opacity-0 group-hover:opacity-100 border border-white/20'}`}
                                                >
                                                    <Check size={16} />
                                                </button>
